...
export function FleetStatus() {
  return (
    <Box className="rounded-lg bg-background border border-border dark:border-white p-4">
      <Text className="border-b border-border dark:border-white pb-1 text-sm font-bold text-primary dark:text-white">
        Fleet Status
      </Text>
      <Box className="mt-2 space-y-1.5">
        {fleetData.map((item) => (
          <HStack key={item.label} className="flex items-center justify-between">
            <Text className="text-primary dark:text-white">{item.label}</Text>
